...
void vector_longest_substring(std::vector<std::string> &vec, std::ostream &ostr, int &output_count) {
  ...
  for(int index=0; index<vec.size(); index++){
    for(int x=0; x < vec[index].size(); x++){
      for(int y=x; y < vec[index].size(); y++){
        std::string sub = vec[index].substr(x,y);
        for(int index_check=0; index_check<vec.size(); index_check++){
          if(index!=index_check && vec[index]!=vec[index_check]){
            int pos= vec[index_check].find(sub);
