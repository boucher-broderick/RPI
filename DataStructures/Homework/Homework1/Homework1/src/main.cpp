...
Data Structures
2/4/2021
The goal of this program is to input various objects and the program will then read the file
that was passed in. The program will then allign the text that was inported in a box that 
was has a specified width. The allignment style is choosen by the user, and the textbox will be 
exported to a file of the users choosing.
*/
...
void full_justify(std::vector<std::vector<std::string>> vec, std::string outfile, int width)
{
    ...
    for (std::vector<std::string> a : vec)
    {
        ...
        if (num_space == 0)
        {
             second = width - total_size+1;
            first = width - total_size+1;
        }
...
/*
  Summary of how I programmed this:
