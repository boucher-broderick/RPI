...
template <class T> void Vec<T>::print() {
  ...
  cout << "Mem Allocated: " << m_alloc << endl;
  cout << "Data:";
...
//O(v*(v-x))
//Removes the variables that are passed through and returns the number of elements removed
template <class T> int remove_mathcing_elements(Vec<T>& v, const T num) {
  int counter=0;
