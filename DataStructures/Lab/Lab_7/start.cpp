...
int recurse(std::vector<std::vector<GRID_STATUS> >& blocked_grid, int& x_, int& y_){
  int p=0;
  int q=0;
  if(x_==0 && y_==0){
    return 1;
  ...
  if(x_>0){
    int x1=x_-1;
    if (blocked_grid[x1][y_] != GRID_BLOCKED){
      p= recurse(blocked_grid, x1, y_ );
    }
  }
  if(y_>0){
    int y1=y_-1;
    if (blocked_grid[x_][y1] != GRID_BLOCKED && y_ >0){
      q=recurse(blocked_grid, x_, y1 );
    }
  }   
  return p+q;
}

int recurse(std::vector<std::vector<int>>& mny, std::vector<std::vector<GRID_STATUS> >& blocked_grid, int& x_, int& y_){
int p=0;
  int q=0;
  if(x_==0 && y_==0){
    return 1;
  ...
  if(x_>0){
    int x1=x_-1;
    if (blocked_grid[x1][y_] != GRID_BLOCKED){
      p= recurse(mny, blocked_grid, x1, y_ );
      if(p==1){
        std::vector<int> i;
        i.push_back(x1);
        i.push_back(y_);
  ...
  if(y_>0){
    int y1=y_-1;
    if (blocked_grid[x_][y1] != GRID_BLOCKED && y_ >0){
      q=recurse(mny, blocked_grid, x_, y1 );
      if(q==1){
        std::vector<int> j;
        j.push_back(x_);
        j.push_back(y1);
...
int main(int argc, char* argv[]) {
  ...
  std::vector<std::vector<int>> mny;
  // Start here with your code...
  std::cout << "Routes: " << recurse(blocked_grid, start_x, start_y ) << std::endl;
  /*std::cout << "Routes: " << recurse(mny, blocked_grid, start_x, start_y ) << std::endl;
  int x[2];
  for (int i=0; i<mny.size(); i++){
    std::cout << mny[i][0] << mny[i][1] << std::endl; 
  }
  */
  return 0;
}
