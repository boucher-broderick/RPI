...
public:
  ...
  public:
    int upcount=0;
    int downcount=0;
    iterator() : ptr_(NULL) root(NULL){}
    iterator(TreeNode* p, TreeNode* r) : ptr_(p) root(r){}
...
private:
  ...
  std::pair<iterator,bool> insert(const T& key_value, TreeNode*& p, TreeNode* the_parent) {
    if (!p) {
      ...
      p->parent = the_parent;
      this->size_++;
      return std::pair<iterator,bool>(iterator(p, root_), true);
    }
      ...
      return std::pair<iterator,bool>(iterator(p, root_), false);
  }
