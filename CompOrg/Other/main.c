...
void ALU_Control(BIT* ALUOp, BIT* funct, BIT* ALUControl)
{
  ...
  ALUControl[0] = or_gate(ALUOp[0], and_gate(ALUOp[1], funct[2]));
  ALUControl[1] = or_gate(and_gate3(ALUOp[1], funct[0], funct[2]), and_gate3(ALUOp[1], funct[1], funct[3]));
...
void updateState()
{
  ...
  ALUOuput[1] = multiplexor2(numberadd, ALUOuput[1], numberadd); 
    ...
    BIT temporary[32];
    for (int x = 0; x < 32; ++x) {
      temporary[x] = multiplexor2(MemToReg, Result[x], ReadData[x]);
    }
