...
BIT multiplexor4(BIT S0, BIT S1, BIT I0, BIT I1, BIT I2, BIT I3)
{
 ...
 BIT one = and_gate(O0, I0);
 BIT two = and_gate(O1, I1);
 BIT three = and_gate(O2, I2);
 BIT four = and_gate(O3, I3);

 return or_gate( or_gate(one, two), or_gate(three, four));
...
void adder(BIT A, BIT B, BIT CarryIn, BIT* CarryOut, BIT* Sum)
{
  	...
  	*Sum = xor_gate(CarryIn, xor_gate(A, B));
...
{
	BIT X = multiplexor2(Binvert, B, not_gate(B));
	BIT res;
	BIT less =0;
	adder(A, X, CarryIn, &CarryOut, &res);
	Result = multiplexor4(Op0, Op1, and_gate(A, X), or_gate(A, X), res, less);
